...
if (!$connect){
    die("Connection failed. check if database exists and try again". $db_connection -> connect_error);
            ...
            if($category == 'MovieMagazine' ||$category == 'Ebook' || $category == 'Media' || $category == 'Stories'){

                    $select = $connect->query("SELECT * FROM publications WHERE category = '".$category." ' ");
                              
                              if ($select->num_rows == 0) {
                                //no entry exists with that serial
                                echo '<div class="text-center" style="color:red;margin-top:200px;"><h3>Oops! <br/> No existing entry in the '.$category.' category. <br/>Pls check back later.</h3></div>';
                                                      ...
                                                      <div class="col-sm-6 col-md-4" >
                                                            <div class="card"><img class="rounded card-img-top w-100 d-block thumbnail_img" src="'.$src.'">
                                                                <div class="card-body">
                                                                    ...
                                                                    <button class="btn btn-primary" type="button" style="border-color:#63c5da;background-color:#63c5da;color: white;width: 100%;" onclick="download_file(\''.$serial.'\')">Download<i class="icon ion-android-download" style="margin-left:12px;"></i></button></div>
                                                            </div>
                                                        </div>
                                                      ';
                                          }
